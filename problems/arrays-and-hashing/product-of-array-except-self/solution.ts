export function productExceptSelf(nums: number[]): number[] {
	const forwardPass: (number | null)[] = Array.from({ length: nums.length }, () => null)
	const backwardPass: (number | null)[] = Array.from({ length: nums.length }, () => null)
	...
	nums.forEach((n, i) => {
		...
		backwardPass[backwardIndex - 1] = nums[backwardIndex] * (backwardPass[backwardIndex] ?? 1)
	})
